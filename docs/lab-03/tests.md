...
| AUTH-01 | API | AC-01 | Valid active login and safe current User | Authenticated session; safe DTO only | `server/tests/lab-03/auth.api.test.ts` | **Pass — Issue #44 candidate `d82a071`; active login/current-user safe projection verified** |
| AUTH-02 | API | AC-05 | Wrong/unknown/inactive/unprovisioned login | Safe failure; no credential/account leak | `server/tests/lab-03/auth.api.test.ts` | **Pass — Issue #44 candidate `d82a071`; wrong/unknown/unprovisioned share safe invalid-credential behavior and inactive guidance requires a correct credential** |
| AUTH-03 | API | AC-02 | Mandatory initial-password gate | Normal capability denied until valid change | `server/tests/lab-03/auth.api.test.ts`; `server/tests/lab-03/authorization.api.test.ts` | **Pass — Issue #44 candidate `d82a071`; pending-password session gate, allowed auth-only endpoints, valid change, session rotation, and old-session revocation verified** |
| AUTH-04 | API | AC-06 | Logout/expiry/reset/role-email-change/deactivation replay | Old protected access rejected | `server/tests/lab-03/auth.api.test.ts` | **Pass — Issue #44 candidate `d82a071`; logout, absolute expiry, password reset/change, auth-version role/email change, and deactivation revoke stale access** |
| AUTH-05 | Unit/API | AC-07 | CSRF/origin/cookie/expiry/throttle policy | Invalid policy requests cannot mutate | `server/tests/lab-03/auth-policy.unit.test.ts`; `server/tests/lab-03/auth.api.test.ts` | **Pass — Issue #44 candidate `d82a071`; CSRF/Origin, exact credentialed CORS, cookie/config, malformed JSON, password policy, and bounded throttling verified** |
| AZ-01 | API | FR-05; AC-03, AC-04 | Role/resource/direct-request matrix | Wrong identity/role/state denied by backend | `server/tests/lab-03/authorization.api.test.ts` | **Pass — Issue #44 candidate `d82a071`; server-derived actor, default-deny capability, Requester ownership, and mandatory-change denial verified** |
...
Verified code/test candidate: `d82a07181a327d96c49b79b47eaf6aaee83458a9` on `feature/44-lab3-auth-foundation` (product implementation commit `04fe7ab` plus strengthened AUTH-04 revocation coverage). This increment implements the backend authentication/session/password-change/authorization foundation only. It intentionally does **not** activate authenticated Requester Ticket/Attachment identity or replace the current browser Development Requester flow; that cross-layer cutover remains Issue #45.

| Check | Actual result |
|---|---|
| `npm.cmd run test:lab3-auth-review` | AUTH-01..AUTH-05 and AZ-01 GREEN: 3 files / 24 tests passed using real Supertest agents and the dedicated PostgreSQL test database. Coverage includes safe login/current-user DTOs, wrong/unknown/inactive/unprovisioned credentials, mandatory password change, password/session rotation, stale-session revocation, logout, absolute expiry, exact Origin/CORS/CSRF policy, malformed-auth JSON safety, bounded throttling, server-derived Actor, role capability denial, and non-disclosing Requester ownership checks. |
| `npm.cmd run verify` | Pass on exact code/test candidate `d82a071`: server 39 files / 213 tests, client 18 files / 123 tests, retained E2E 23/23, dedicated responsive 10/10; server/client builds passed. |

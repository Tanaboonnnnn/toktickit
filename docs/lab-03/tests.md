...
PR #64 was accidentally merged into `lab3-staging` before the final presentation-data cleanup/re-review was finished. The follow-up does not revert valid CI/evidence infrastructure or rewrite history. It replaces technical fixture labels visible in screenshots with realistic human/product content, adds a release-evidence guard against leaked fixture tokens, and refreshes the repository-visible screenshot set.

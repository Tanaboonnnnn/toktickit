...
function AttachmentCard({ attachment, removing, busy, onDownload, onRemove, onCancel, reason, setReason, reasonError, confirmRemove }: { attachment: TicketAttachmentMetadata; removing: boolean; busy: boolean; onDownload: () => void; onRemove: () => void; onCancel: () => void; reason: string; setReason: (v: string) => void; reasonError: string; confirmRemove: () => void }) {
  return <article className="lab2-attachment-card"><h3 className="lab2-filename">{attachment.originalName}</h3><dl><dt>Type</dt><dd>{attachment.mimeType}</dd><dt>Size</dt><dd>{formatAttachmentSize(attachment.sizeBytes)}</dd><dt>Uploaded</dt><dd>{formatDate(attachment.createdAt)}</dd><dt>State</dt><dd>{attachment.state === "REMOVED" ? "Removed" : "Active"}</dd>{attachment.state === "REMOVED" && <><dt>Removed</dt><dd>{attachment.removedAt ? formatDate(attachment.removedAt) : "เนโฌโ€"}</dd><dt>Removal reason</dt><dd>{attachment.removalReason}</dd></>}

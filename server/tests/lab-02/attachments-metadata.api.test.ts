...
import { createAttachmentFixture, destroyAttachmentFixture, bytes, listAsRequester, upload, type AttachmentFixture } from "./attachment-test-fixture.js";
...
describe("API-15 Attachment metadata", () => {
  it("returns active and removed metadata in createdAt/id order without internals", async () => {
    ...
    const r = await listAsRequester(f, f.requesterA.id, f.ticketId);

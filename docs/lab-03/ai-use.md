...
Issue #41 established the Lab 3 engineering contract and planning authority, and Issues #41/#42 have since completed their recorded human-review/merge gates. Issue #43 is the first Lab 3 product increment: it evolves the existing Lab 2 schema in place rather than replacing historical data, introduces password primitives and local provisioning without activating login yet, and makes the current Requester runtime tolerant of all eight Lab 3 statuses before mixed-status fixtures become visible. Only the Issue #43 Test DD rows actually exercised by this increment are promoted from planned language; later authentication/staff/admin tests remain planned.

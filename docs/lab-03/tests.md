...
| UI-04 | UI | FR-12–FR-15; AC-15–AC-18 | Staff Detail controls/confirm/conflict | Only permitted actions; safe conflict handling | `client/tests/lab-03/StaffTicketDetail.test.tsx` | **Pass — Issue #48: 5/5 component cases cover Claim, contextual confirmed reassignment, independent IT Priority editing, permitted next-status controls/required fields, and `409` authoritative reload with no automatic mutation replay. Fresh full client verification passed 21 files / 120 tests.** |
...
| `client/tests/lab-03/StaffTicketDetail.test.tsx` | Pass — **5/5 UI-04 tests** for Claim, contextual confirmed reassignment, IT Priority mutation while Requested Priority remains read-only, only permitted next statuses/required fields, and `409 CONFLICT` authoritative reload without automatic replay. |
...
| `npm.cmd run verify` | Pass on pre-review candidate `1f87ff4`: server build + **44 files / 253 tests**; client production build + **21 files / 120 tests**; full Chromium E2E **29/29**; retained dedicated responsive rerun **10/10**. |
